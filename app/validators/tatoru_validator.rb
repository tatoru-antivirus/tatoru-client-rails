...
class TatoruValidator < ActiveModel::EachValidator
  def validate_each(record, attribute, value)
    record.errors.add(attribute, :contains_malware) if infected?(value)
  end
  ...
  def validate_each(record, attribute, _value)
    ...
    files.each do |file|
      ...
      if scan.result["status"] == "clean"
        ...
        add_error(record, attribute, options[:message].presence || :contains_malware,
                  matches: scan.result["matches"].to_sentence)
        break
      end
    end
  end
